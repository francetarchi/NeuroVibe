...
class MainActivity : AppCompatActivity() {
    ...
    private fun startServerManager() {
        ...
        buttonStartEEG.setOnClickListener {
            ...
            // Mostro stato della batteria del MindRove
            val batteryString = if(estimateBatteryPercentage(voltage) == 120){
                "Charging..."
            } else {
            ...
            completeRound = if(currentImageIndex == 0) completeRound + 1 else completeRound
    ...
    private fun estimateBatteryPercentage(voltage: UInt): Int{
        val table = listOf(
            4200 to 100,
            4100 to 90,
            4000 to 80,
        ...
        for (i in 0 until table.size - 1) {
            ...
            if(voltage in v2.toUInt()..v1.toUInt()){
                val fraction = (voltage - v2.toUInt()).toDouble() / (v1 - v2)
                val percentage = p2 + fraction * (p1 - p2)
                Log.d("Battery", "Voltage: $voltage, Percentage: $percentage")
                return percentage.toInt().coerceIn(0, 100)
            }
        }
        return if(voltage >= 4200u) 120 else 0
    }
}
